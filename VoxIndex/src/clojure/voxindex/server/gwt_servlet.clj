...
#_ (* Constructs a list of (uri, source ID) pairs for use in looking
      ...
      @(returns A list whose elements are of the form 
                @(form [service-uri source-id]), where
                @arg service-uri is a source's service ID
                @arg source-id is the corresponding source object ID.)
      )
(defn make-service-uri-list [db env]
  (let [sources (fetch-all db (source-collection db))]
    (reduce 
      (fn [uri-list+ source]
        (let [uri (mapped-source-uri env source false)] 
          #_(println "m-s-uri: " source " = " uri)
          (conj uri-list+ [uri (id-of source)])))
      '() 
      sources)))

(defn uri-source [source-list uri]
  (let [[source-uri source-id] 
        (some (fn [[source-uri id]] 
                (if (or (.equals source-uri uri) 
                        (.startsWith uri (str source-uri "/")))
                  [source-uri id])) 
              source-list)]
    (if source-id
      (let [[_ rel-uri] (re-matches #"/*(.*)" (subs uri (.length source-uri)))]
        [source-id rel-uri]))))

#_ (* Pitiful mechanism for finding an indexable that corresponds to a given 
     relative URI. 
)
(defn uri-indexable [db source-list uri]
  (let [[source-id rel-uri] (uri-source source-list uri)]
    (if source-id
      (let [indexable (fetch-one db (indexable-collection db) 
                                 { "source-ref" source-id "relative-uri" rel-uri} )]
        indexable))))
            ...
            (let [indexable (indexable-of target)
                  ...
                  (LookupResult$Indexable. 
                    ...
                    (if source
                      ...
                      "")
                    (mapped-uri env source (relative-uri-of indexable) false)
	      ...
	      (try
	        (do
	          ...
	          (let [db (session-db-of session)
	                uri-list (make-service-uri-list db env)
